...
<#macro ACTION statement service>
            ...
            if (action->state == STOP) 
            {
                ...
                {
                    UFPort_WriteToBuffer(action->groupPort.portId, (unsigned char*) service_list[service_index].group, sizeof(int32), 0, &dataLen);
                }

                UFControl_RunTask(CONTROL_TASK_ID, action->action_task_id);
                action->state = RUN;
                LOG_DEBUG("New action (task %s)", , action->task_name);
            }
...
<#macro THROW statement service>
            int32 event = ID_EVENT_${statement.statement.statement.event.name};
            event_list[event] = TRUE;
            <#if statement.statement.statement.isBroadcast() == true>
            UFPort_WriteToQueue(throw_out_port_of_${statement.statementId}.port->portId, (unsigned char*) &event, sizeof(int32), 0, &dataLen);
            </#if>
</#macro>