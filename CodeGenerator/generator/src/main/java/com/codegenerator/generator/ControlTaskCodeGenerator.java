...
public class ControlTaskCodeGenerator {
        ...
        private void generateEventCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.EVENT_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.EVENT_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.EVENT_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.EVENT_SOURCE_SUFFIX),
                                rootHash);
        }
        ...
        private void generatePortCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                commPortList.addAll(
                                robot.getRobot().getRobotTask().getControlTask().getOutputPortList(
                                                robot.getRobot().getRobotTask().getReportTask()));
                ...
                rootHash.put(ControlTaskConstant.LEADER_PORT,
                                robot.getRobot().getRobotTask().getControlTask().getInputPortList(
                                                robot.getRobot().getRobotTask().getLeaderTask())
                                                .get(0));

                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.PORT_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.PORT_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.PORT_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.PORT_SOURCE_SUFFIX),
                                rootHash);
        ...
        private void generateResourceCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                rootHash.put(ControlTaskConstant.ROBOT_ID, robot.getRobotName());
                rootHash.put(ControlTaskConstant.RESOURCE_LIST, robot.getRobot().getRobotTask()
                                .getRobot().getRobotType().getRobotType().getResourceList());

                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.RESOURCE_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.RESOURCE_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.RESOURCE_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.RESOURCE_SOURCE_SUFFIX),
                                rootHash);
        }
        ...
        private void generateActionCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                robot.getModeList().forEach(m -> {
                        m.getServiceList().forEach(service -> {
                                service.getStatementList().forEach(st -> {
                                        actionTaskList.addAll(st.getActionList().stream()
                                                        .map(a -> a.getActionTask())
                                                        .collect(Collectors.toList()));
                                        actionTypeSet.addAll(st.getActionList().stream()
                                                        .map(a -> a.getActionTask().getActionImpl()
                                                                        .getActionType())
                                                        .collect(Collectors.toList()));
                                });
                        });
                ...
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.ACTION_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.ACTION_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.ACTION_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.ACTION_SOURCE_SUFFIX),
                                rootHash);
        ...
        private void generateTimerCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                for (CodeModeWrapper mode : robot.getModeList()) {
                        for (CodeServiceWrapper codeService : mode.getServiceList()) {
                                for (CodeActionWrapper action : codeService.getActionList()) {
                                        if (action.getActionTask().getActionStatement()
                                                        .getDeadline() != null) {
                                                timerCount++;
                                        }
                                }
                                for (CodeStatementWrapper statement : codeService
                                                .getConditionStatementList()) {
                ...
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.TIMER_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.TIMER_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.TIMER_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.TIMER_SOURCE_SUFFIX),
                                rootHash);
        }
        ...
        private void generateServiceCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.SERVICE_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.SERVICE_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.SERVICE_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.SERVICE_SOURCE_SUFFIX),
                                rootHash);
        }
        ...
        private void generateModeCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.MODE_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.MODE_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.MODE_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.MODE_SOURCE_SUFFIX),
                                rootHash);
        }
        ...
        private void generateTransitionCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.TRANSITION_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.TRANSITION_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.TRANSITION_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.TRANSITION_SOURCE_SUFFIX),
                                rootHash);
        }
                ...
                try {
                        LocalFileCopier.copyFile(CodeGeneratorConstant.CONTROL_TASK_CODE,
                                        Paths.get(targetDir.toString(),
                                                        CodeGeneratorConstant.CONTROL_TASK_CIC));
                } catch (Exception e) {
                        e.printStackTrace();
                }
        }
