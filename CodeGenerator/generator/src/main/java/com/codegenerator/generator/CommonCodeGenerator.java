...
public class CommonCodeGenerator {
                ...
                try {
                        LocalFileCopier.copyFile(CodeGeneratorConstant.COMMON_HEADER_CODE,
                                        Paths.get(targetDir.toString(),
                                                        CodeGeneratorConstant.COMMON_HEADER));
                        LocalFileCopier.copyFile(CodeGeneratorConstant.LOGGER_HEADER_CODE,
                                        Paths.get(targetDir.toString(),
                                                        CodeGeneratorConstant.LOGGER_HEADER));
                } catch (Exception e) {
                        e.printStackTrace();
                }
        }
        ...
        private void generateRobotSpecificCommon(Path targetDir, UEMRobot robot) {
                ...
                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.ROBOT_SPECIFIC_COMMON_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.COMMON_HEADER_SUFFIX),
                                rootHash);
        }
        ...
        private List<VariableTypeWrapper> makeVariableTypeList(CodeRobotWrapper robot) {
                ...
                for (VariableTypeWrapper variableType : robot.getRobot().getRobotTask().getRobot()
                        ...
                        if (variableTypeList.stream()
                                        .filter(v -> v.getVariableType().getName().equals(
                                                        variableType.getVariableType().getName()))
                                        .count() == 0) {
        ...
        private void generateVariableCode(Path targetDir, CodeRobotWrapper robot) {
                ...
                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.VARIABLE_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.VARIABLE_HEADER_SUFFIX),
                                rootHash);
                FTLHandler.getInstance().generateCode(
                                CodeGeneratorConstant.VARIABLE_SOURCE_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.VARIABLE_SOURCE_SUFFIX),
                                rootHash);
        }
        ...
        private void generateGroupCode(Path targetDir, CodeRobotWrapper robot) throws Exception {
                ...
                FTLHandler.getInstance().generateCode(CodeGeneratorConstant.GROUP_HEADER_TEMPLATE,
                                Paths.get(targetDir.toString(), robot.getRobotName()
                                                + CodeGeneratorConstant.GROUP_HEADER_SUFFIX),
                                rootHash);
        ...
        private void copyVariableCode(Path targetDir) {
                try {
                        LocalFileCopier.copyFile(CodeGeneratorConstant.VARIABLE_HEADER_CODE,
                                        Paths.get(targetDir.toString(),
                                                        CodeGeneratorConstant.VARIABLE_HEADER));
                        LocalFileCopier.copyFile(CodeGeneratorConstant.VARIABLE_SOURCE_CODE,
                                        Paths.get(targetDir.toString(),
                                                        CodeGeneratorConstant.VARIABLE_SOURCE));
                } catch (Exception e) {
                        e.printStackTrace();
                }
        }
