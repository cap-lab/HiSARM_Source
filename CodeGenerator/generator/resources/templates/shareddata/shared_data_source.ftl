...
semo_int8 ${libName}_listen_${libName}_updated;
STreadMutex listen_${libName}_mutex;

VARIABLE_${libName} action_${libName};
semo_int8 action_${libName}_updated;
...
LIBFUNC(void, set_${libName}_listen, semo_int32 sender_robot, void *buffer) {
...
LIBFUNC(void, get_${libName}_action, semo_int32 *sender_robot, void *buffer) {
    UCTreadMutex_Lock(listen_${libName}_mutex);
    listen_${libName}_updated = FALSE;
...
LIBFUNC(semo_int8, avail_${libName}_action) {
...
LIBFUNC(semo_int8, avail_${libName}_report) {
