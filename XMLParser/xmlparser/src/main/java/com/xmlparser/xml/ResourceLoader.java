...
import com.xmlparser.ObjectFactory;
...
public abstract class ResourceLoader<S> {
	...
	private void initUnmarshaller(String xsdFile, Unmarshaller unmarshaller) throws JAXBException {
		...
		unmarshaller.setEventHandler(new ValidationEventHandler() {
			...
			public boolean handleEvent(ValidationEvent ve) {
				...
				if (ve.getSeverity() != ValidationEvent.WARNING) {
					...
					logger.warning("Line:Col[" + vel.getLineNumber() + ":" + vel.getColumnNumber()
							+ "]:" + ve.getMessage());
					return false;
				}
				return true;
	...
	public void storeResource(S resource, Writer writer) throws UEMXMLException {
		try {
			...
			throw new UEMXMLException(UEMXMLErrorCode.XML_MARSHAL_ERROR, e);
		} finally {
			if (writer != null)
				try {
					writer.close();
	...
	public void storeResource(S resource, OutputStream os) throws UEMXMLException {
		storeResource(resource, new OutputStreamWriter(os));
	}

	public S loadResource(String fileName) throws UEMXMLException {
		InputStream is;
		try {
			is = new FileInputStream(fileName);
		} catch (IOException e) {
			throw new UEMXMLException(UEMXMLErrorCode.FILE_OPEN_ERROR, e);
		}
	...
	public S loadResource(InputStream is) throws UEMXMLException {
		JAXBElement<S> element = null;
		try {
			element = (JAXBElement<S>) getUnmarshaller().unmarshal(is);
		} catch (JAXBException e) {
			throw new UEMXMLException(UEMXMLErrorCode.XML_UNMARSHAL_ERROR, e);
		}
